...
namespace CS2MultiplayerMod.Game.Sync.Systems
{
    public partial class NameSyncSystem
    {
        private void ApplyIncoming(MultiplayerSession session, long now)
        {
            ...
            if (_targetRetry.Count > 0 && now - _lastRetryMs >= RetryIntervalMs)
            {
                _lastRetryMs = now;
                RetryPending(now);
            }
            ...
            while (_incoming.TryDequeue(out message))
            {
                ...
                if (!TryApplyName(command, message.OriginPlayerId))
        ...
        private void RetryPending(long now)
        {
            for (int i = 0; i < _targetRetry.Count;)
            {
                ...
                if (TryApplyName(pending.cmd, pending.origin))
                {
        ...
        /// </summary>
        private bool TryApplyName(EntityNameCommand command, int origin)
        {
